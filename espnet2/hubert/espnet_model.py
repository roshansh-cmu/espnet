...
from espnet2.hubert.resnet import ResEncoder
...
class AVHubertPretrainModel(AbsESPnetModel):
    """Hubert Pretrain model"""
    ...
    def __init__(
        ...
        pred_nomask_weight: float = 0.0,
        loss_weights: float = 0.0,
        resnet_relu_type: str = None,
        resnet_weights: str = None,
        video_frontend: str = None,
        ...
        self.video_frontend = (
            (
                ResEncoder(resnet_relu_type, resnet_weights)
                if video_frontend == "resnet"
                else torch.nn.Linear(2048, 512)
            )
            if video_frontend is not None
            else None
        )
        self.specaug = specaug
        self.normalize = normalize
        ...
        if report_cer or report_wer:
            ...
            self.error_calculator = None
        self.fusion_type = video_fusion_type
    ...
    def forward(
        ...
        **kwargs,
        ...
        Args:
            ...
            text: (Batch, Length)
            text_lengths: (Batch,)
            video: (Batch,Length,..) Optional
            video_lengths: (Batch,) Optional
        """
        assert text_lengths.dim() == 1, text_lengths.shape
        # Check that batch_size is unified
        assert (
        ...
        # 1. Encoder
        encoder_out = self.encode(
            speech, speech_lengths, text, text_lengths, video, video_lengths
        )
    ...
    def encode(
        ...
        encoder_out = self.encoder(
            feats, feats_lengths, y_pad, y_pad_length, video, video_lengths
        )
