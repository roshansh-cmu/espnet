...
class AVHubertConfig(FairseqDataclass):
    ...
    video_extractor: str = field(
        default="resnet",
        metadata={"help": "Feature extractor for video- resnet or linear"},
    )
    video_feat_dim: int = field(
        default=2048, metadata={"help": "audio feature dimension"}
    )
...
class AVHubertModel(BaseFairseqModel):
    def __init__(
        ...
        **kwargs,
        ...
        self.feature_extractor_video = (
            SubModel(resnet=resnet, input_dim=resnet.backend_out, cfg=sub_cfg)
            if cfg.video_extractor == "resnet"
            else SubModel(resnet=None, input_dim=cfg.video_feat_dim, cfg=sub_cfg)
        )
        ...
        self.fuse_dimension = cfg.fuse_dimension
