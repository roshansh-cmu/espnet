...
for k in data:
    if "abst" not in data[k]:
        ...
        continue
    summ_dict[k] = (
        data[k]["title"].strip().encode("ascii", "ignore").decode().replace("\n", " ")
        + " [SEP] "
        + data[k]["abst"].strip().encode("ascii", "ignore").decode().replace("\n", " ")
    )
...
valid_key_dict = {
    k: 1 for k in key_arr[int(0.8 * len(key_arr)) : int(0.9 * len(key_arr))]
}
