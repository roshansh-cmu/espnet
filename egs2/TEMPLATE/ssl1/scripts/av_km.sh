...
km_path_vid="${kmrootdir}/km_${train_set}_${feature_type}/km_vid_${nclusters_vid}clusters.mdl"

mkdir -p "$(dirname ${km_path})"

if [ ${stage} -le 1 ] && [ ${stop_stage} -ge 1 ]; then
    log "stage 1: Learn K-means with ${feature_type} feature based on scikit-learn"
    ## First Run on Audio, then video
    ...
    ${python} pyscripts/sklearn_km.py \
              --feats-dir "${datadir}/${train_set}" \
              --km-path "${km_path_vid}" \
              --n-cluster "${nclusters_vid}" \
              --feature-type "extracted" \
              --feature_path "${datadir}/${train_set}/vad.scp" \
              --hubert-model-url "${hubert_url}" \
              --hubert-model-path "${hubert_dir_path}" \
              --nj ${nj} \
              --portion ${portion}
...
if [ ${stage} -le 2 ] && [ ${stop_stage} -ge 2 ]; then
    ...
    for task in ${dev_set} ${test_set}; do
...
#	#	${python} pyscripts/dump_km_label.py \
	...
	#fi	
        ${python} pyscripts/dump_km_label.py \
                  --km-path "${km_path_vid}" \
                  --label-path "${plabel_dir}/text_vid" \
                  --recog-set "${plabel_dir}" \
                  --feature "extracted" \
                  --offset "${nclusters}" \
                  --feature_path "${plabel_dir}/vad.scp" \
                  --nj ${nj}
        cp "${plabel_dir}/text" "${plabel_dir}/text_audio"
        paste -d ' ' <( LC_ALL=C sort "${plabel_dir}/text") <( LC_ALL=C sort "${plabel_dir}/text_vid" | cut -d ' ' -f 2- ) > tmp && mv tmp "${plabel_dir}/text"
