...
if ! "${skip_train}"; then
    ...
    if [ ${stage} -le 10 ] && [ ${stop_stage} -ge 10 ]; then
        ...
        ${train_cmd} JOB=1:"${_nj}" "${_logdir}"/stats.JOB.log \
            ${python} -m espnet2.bin.asr_train \
                ...
                --train_data_path_and_name_and_type "${_asr_train_dir}/video_${video_feats_type}.scp,video,kaldi_ark" \
                --train_data_path_and_name_and_type "${_asr_train_dir}/text,text,text" \
                --valid_data_path_and_name_and_type "${_asr_valid_dir}/${_scp},speech,${_type}" \
                --valid_data_path_and_name_and_type "${_asr_valid_dir}/video_${video_feats_type}.scp,video,kaldi_ark" \
    ...
    if [ ${stage} -le 11 ] && [ ${stop_stage} -ge 11 ]; then
        ...
        if [ "${num_splits_asr}" -gt 1 ]; then
            ...
            _opts+="--train_data_path_and_name_and_type ${_split_dir}/video_${video_feats_type}.scp,video,kaldi_ark "
            ...
            _opts+="--train_data_path_and_name_and_type ${_asr_train_dir}/video_${video_feats_type}.scp,video,kaldi_ark "
        ...
        for dset in ${test_sets}; do
            ...
            ${_cmd} --gpu "${_ngpu}" JOB=1:"${_nj}" "${_logdir}"/asr_inference.JOB.log \
                ${python} -m ${asr_inference_tool} \
                    ...
                    --data_path_and_name_and_type "${_data}/video_${video_feats_type}.scp,video,kaldi_ark" \
