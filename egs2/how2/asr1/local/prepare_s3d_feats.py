...
dsets = ["train_subset_50", "dev5_test"]

# feature_sets = ["r2plus1d", "resnext32", "vit_l16"]

feature_sets = ["vit_l16"]
feature_sets = ["s3d"]

BATCH_EVERY = 140
VIDEO_SUBSAMPLING_FACTOR = 3
...
norm_tf = torchvision.transforms.Normalize()
        ...
        for feature_set in feature_sets:
            ...
            with WriteHelper(
                ...
                for line in tqdm.tqdm(lines):
                    ...
                    for (utt_id, start, stop) in vid2segments[video_id]:
                        ...
                        if vid_input.shape[0] > BATCH_EVERY:
                            for i in range(0, vid_input.shape[0], BATCH_EVERY):
                                ...
                                elif feature_set == "s3d":
                                    vid_segment = vid_segment.unsqueeze(0)
                                    vid_segment = vid_segment.permute(0, 2, 1, 3, 4)
                                    vid_segment = normalize(vid_segment)
                                with torch.no_grad():
                            ...
                            output = torch.cat((output["video_embedding"],output['mixed_5c']),dim=-1)
                        else:
                            with torch.no_grad():
                                if torch.cuda.is_available():
                                    ...
                                    vid_input = vid_input.permute(0, 2, 1, 3, 4)
                                    vid_input = normalize(vid_input)
                                ...
                                output = torch.cat((output["video_embedding"],output['mixed_5c']),dim=-1)
                        print(f"JOB {job_index} | Processed video {utt_id} in {time.time()-st_time}s | OP Shape {output.shape}")
                    ...
                    if i % 500 == 0:
                        print(
                            f"JOB {job_index} | Finished {i}/{nlines} videos | {nutt} segments"
                        )
